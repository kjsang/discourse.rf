...
# 정보원 확인: 찬성 연합 중 광역자치단체 빈도 (초기)
data_tb %>% 
  filter(시기 == "초기") %>% 
  mutate(연합구분 = ifelse(정보원 %in% c("청와대", "문재인대통령","민주당_이인영", "민주당"), "중립", 
                          ifelse(정보원 %in% c("민주당_김경수_광역자치단체", "민주당_이재명_광역자치단체", "민주당_박원순_광역자치단체", "정의당", "전문가집단_윤형중", "광역자치단체_대구시"), "찬성", "반대"))) %>% 
  filter(정보원 %in% c("청와대", "문재인대통령", "민주당_이인영", "민주당", "민주당_김경수_광역자치단체", "민주당_이재명_광역자치단체", "민주당_박원순_광역자치단체", "정의당", "전문가집단_윤형중", "기획재정부_홍남기", "통합당_심재철", "기획재정부", "통합당_황교안", "국무총리_정세균", "광역자치단체_대구시")) %>% 
  filter(연합구분 == "찬성") %>% 
  count(정보원) %>% 
  arrange(desc(n)) %>% 
...
data_tb %>% 
  ...
  filter(정보원 %in% c("청와대", "문재인대통령", "민주당_이인영", "민주당", "민주당_김경수_광역자치단체", "민주당_이재명_광역자치단체", "민주당_박원순_광역자치단체", "정의당", "전문가집단_윤형중", "기획재정부_홍남기", "통합당_심재철", "기획재정부", "통합당_황교안", "국무총리_정세균", "광역자치단체_대구시")) %>% 
  filter(정보원 %>% str_detect("광역자치단체")) %>% 
  filter(정보원 == "광역자치단체_대구시") %>% 
  as.data.frame()

# 정보원 확인: 찬성 연합 중 광역자치단체 빈도 (중기)
data_tb %>% 
  filter(시기 == "중기") %>% 
  mutate(연합구분 = ifelse(정보원 %in% c("청와대", "문재인대통령", "민주당_김경수_광역자치단체", "민주당_이재명_광역자치단체", "정의당", "민주당", "민주당_이인영", "통합당_황교안", "민주당_이해찬", "민주당_이낙연", "광역자치단체_경기도", "민주당_박원순_광역자치단체", "민생당", "민주당_김경수_광역자치단체", "국무총리_정세균"), "찬성", 
                          ifelse(정보원 %in% c("통합당_김종인", "통합당_유승민", "통합당_박형준", "통합당", "기획재정부_홍남기", "기획재정부"), "반대", "중립"))) %>%
...
data_tb %>% 
  filter(시기 == "중기") %>% 
  mutate(연합구분 = ifelse(정보원 %in% c("청와대", "문재인대통령", "민주당_김경수_광역자치단체", "민주당_이재명_광역자치단체", "정의당", "민주당", "민주당_이인영", "통합당_황교안", "민주당_이해찬", "민주당_이낙연", "광역자치단체_경기도", "민주당_박원순_광역자치단체", "민생당", "민주당_김경수_광역자치단체", "국무총리_정세균", "광역자치단체_대전시", "광역자치단체_경상남도", "광역자치단체_부산시", "광역자치단체_인천시", "광역자치단체_제주시", "광역자치단체_대구시", "광역자치단체_울산시"), "찬성", 
                          ifelse(정보원 %in% c("통합당_김종인", "통합당_유승민", "통합당_박형준", "통합당", "기획재정부_홍남기", "기획재정부"), "반대", "중립"))) %>%
  filter(연합구분 == "찬성") %>% 
  count(정보원) %>% 
  arrange(desc(n)) %>% 
...
# 정보원 확인: 찬성 연합 중 광역자치단체 빈도 (후기)
data_tb %>% 
  filter(시기 == "후기") %>% 
  mutate(연합구분 = ifelse(정보원 %in% c("청와대", "문재인대통령", "민주당_김경수_광역자치단체", "민주당_이재명_광역자치단체", "광역자치단체_인천시", "광역자치단체_부산시", "광역자치단체_울산시", "광역자치단체_제주시", "광역자치단체_경상남도", "광역자치단체_대구시", "정의당", "민주당", "민주당_이인영", "통합당_황교안", "민주당_이해찬", "민주당_이낙연", "광역자치단체_경기도", "민주당_박원순_광역자치단체", "민생당", "민주당_김경수_광역자치단체", "국무총리_정세균", "민주당_이근형", "민주당_박주민", "민주당_조정식", "정부", "더불어시민당"), "찬성", 
                          ifelse(정보원 %in% c("통합당_조경태", "통합당_심재철", "통합당_김종인", "통합당_유승민", "통합당_박형준", "통합당", "기획재정부_홍남기", "기획재정부"), "반대", "중립"))) %>% 
  filter(연합구분 == "찬성") %>% 
  count(정보원) %>% 
  arrange(desc(n)) %>% 
...
data_freq %>% 
  mutate(ratio = (광역자치단체/찬성)*100,
         "찬성연합 중 광역자치단체 비율" = paste0(round(ratio, 2), "%")) %>% 
  select(시기, 전체, 찬성, 광역자치단체, `찬성연합 중 광역자치단체 비율`)
